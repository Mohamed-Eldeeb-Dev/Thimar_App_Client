...
import 'package:thimar_app/features/auth/verify_of_reset_pass/cubit/verify_of_reset_password_cubit.dart';

class VerifyOfResetPassword extends StatefulWidget {
  const VerifyOfResetPassword({super.key, required this.phone});
  final String phone;

  @override
  State<VerifyOfResetPassword> createState() => _VerifyOfResetPasswordState();
}

class _VerifyOfResetPasswordState extends State<VerifyOfResetPassword> {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => VerifyOfResetPasswordCubit(),
      child: Form(
        key: _formKey,
        child: Scaffold(
          body:
              BlocConsumer<
                VerifyOfResetPasswordCubit,
                VerifyOfResetPasswordState
              >(
                listener: (context, state) {
                  if (state is VerifyOfResetPasswordSuccess) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('تم التحقق بنجاح')),
                    );
                    AppRoutesFunc.pushAndRemoveUntil(NamedRoutes.resetPassword);
                  } else if (state is VerifyOfResetPasswordFailure) {
                        ...
                        CustomElevatedButton(
                          onPressed: () {
                            if (_formKey.currentState!.validate()) {
                              context.read<VerifyOfResetPasswordCubit>().verify(
                        ...
                        OutlinedButton(
                          ...
                          onPressed: _isResendEnabled
                              ? () {
                                  if (_formKey.currentState!.validate()) {
                                    context
                                        .read<VerifyOfResetPasswordCubit>()
