...
import 'package:thimar_app/core/services/loger.dart';
...
class Model {
  ...
  String stringFromJson(
    ...
    String attribute, {
    String defaultValue = '',
    List<String>? otherAttribute,
  }) {
    try {
      if (json == null) {
        return defaultValue;
      } else {
        List<String> attributes = [attribute] + (otherAttribute ?? []);
        for (var i = 0; i < attributes.length; i++) {
          if (json[attributes[i]] != null) {
            return (json[attributes[i]] ?? defaultValue).toString();
          }
        }
      }
      return defaultValue;
  ...
  DateTime dateFromJson(
    ...
    try {
      if (json == null || json[attribute] == null) {
        ...
        return DateFormat(parseingFormat, 'en').parse("${json[attribute]}");
      } else {
